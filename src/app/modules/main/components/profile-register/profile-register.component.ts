...
export class ProfileRegisterComponent implements OnInit {
  ...
  onProfileSubmit = (formDirective: FormGroupDirective) => {
    ...
    for(let i = 0; i < myCheckboxes.length; i++) {
      if(myCheckboxes[i].checked){
        let obj = {"menus_id": myCheckboxes[i].name, "rules_id": myCheckboxes[i].value};
  ...
  listMenuNotTrashed = () => {
    ...
    this._crud.read({route: 'profiles/rules/not-trashed'}).then(res => {
      this.rules = res['obj'];
      rules = res['obj'];
      this._crud.read({route: 'profiles/menus/not-trashed'}).then(res => {  
        this.menus = res['obj'];  
        for(let i = 0; i < this.menus.length; i++){ 
          ...
          for(let index = 0; index < rules.length; index++){
            ...
            for(let j = 0; j < this.permissionEdit.length; j++){                
              if(this.permissionEdit[j].rule_id ==  this.menus[i].rules[index].id && this.menus[i].id == this.permissionEdit[j].menu_id){
